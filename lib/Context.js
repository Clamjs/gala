...
util.inherits(Context, {
  ...
  pipe: function (file, charset, type) {
    ...
    this.status = 200;
    var assetsRoot = UnderPath(this.rootdir, this.options.assets.rootdir);
    file = UnderPath(assetsRoot, file || Url.parse(this.url).pathname);
    try {
      var options = this.options;
  ...
  render: function (file, charset) {
    ...
    var res = this.res;
    var options = this.options;
    var viewRoot = UnderPath(this.rootdir, options.view.rootdir);
    var tplFile = UnderPath(viewRoot, file || this.urlinfo.pathname);
