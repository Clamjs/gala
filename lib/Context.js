...
function Context (req, res, app) {
  ...
  var options = this.options = util.merge({
    rootdir: process.cwd()
  }, config, app.options);
  this.alias = {
    view: Path.join(options.rootdir, options.view.rootdir),
    assets: Path.join(options.rootdir, options.assets.rootdir),
    model: Path.join(options.rootdir, options.model.rootdir),
    controller: Path.join(options.rootdir, options.controller.rootdir)
  };
  ...
  var extname = this.extname = Path.extname(req.url);
  var engine = options.view.supports[extname];
  if (options.view.supports[extname]) {
    this.type = 'view';
  } else if (options.assets.supports[extname]) {
    this.type = 'assets';
  } else {
    this.type = 'others';
  }
...
util.inherits(Context, {
  ...
  pipe: function (file, charset, type) {
    ...
    this.status = 200;
    file = UnderRoot(this.alias.assets, file || Url.parse(this.url).pathname);
    try {
      var options = this.options;
      if (this.type === 'assets') {
        charset = '; charset=' + (charset || options.assets.charset || 'UTF-8');
      } else {
        charset = '';
      }
  ...
  error: function (msg) {
    ...
    res.write(Context.engine('juicer').render(content, {
      __STATUS: status,
      __DEBUG: this.DEBUG,
      message: err.message,
  ...
  render: function (file, charset) {
    this.status = 200;
    var res = this.res;
    file = UnderRoot(this.alias.view, file || Url.parse(this.url).pathname);
    try {
      ...
      var options = this.options;
      var content = Context.engine('juicer').render(Fs.readFileSync(file).toString(), scope);
