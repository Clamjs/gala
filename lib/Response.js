...
util.Klass(Response, {
    ...
    pipe: function (file, charset) {
        ...
        if (fs.existsSync(file)) {
            ...
            if (MIME.match(/javascript|text|json/)) {
                this._headers['Content-Type'] += "; charset=" + (charset || this.charset || "utf-8");
            }
    ...
    html: function(content, charset) {
        var res = this.res;
        try {
            res.writeHead(200, util.mix(this._headers, {
                'Content-Type': 'text/html; charset=' + (charset || this.charset || "utf-8")
            }));
            res.end(content);
        } catch (e) {
            return this.error(e);
    ...
    json: function (scope, charset) {
        var res = this.res;
        try {
            scope = JSON.stringify(scope || {});
            res.writeHead(200, util.mix(this._headers, {
                'Content-Type': 'application/json; charset=' + (charset || this.charset || "utf-8")
            }));
            res.end(scope);
        } catch (e) {
            return this.error(e);
    ...
    jsonp: function (scope, charset) {
        var res = this.res;
        var jsonp = this.ctx.query[this.options.jsonp];
